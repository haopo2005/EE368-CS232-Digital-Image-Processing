...
def draw_avgflow(img, avgflow):
    ...
    lines = np.vstack([x, y, x+fx, y+fy]).T.reshape(-1, 2, 2)
    lines = np.int32(lines + 0.5)
    cv2.polylines(img, lines, 0, (0,0,255), thickness=2)
    for (x1, y1), (x2, y2) in lines:
...
def predSpeed(im, prev, cur, labels, **options):
    ...
    with open('{0}/parameters.txt'.format(parampath), 'r') as paramfile:
        ...
        coef_speed = np.fromiter(map(float,coef_speed), dtype=np.float32)
        coef_angle = paramfile.readline().split(',')
        coef_angle = np.fromiter(map(float, coef_angle), dtype=np.float32)
    options['rseg'] = rseg
    options['cseg'] = cseg
    flow = compFlow(prev, cur, **options) #flow --> list
...
def main():
    parser = argparse.ArgumentParser(description='detect optical flow from image')
    parser.add_argument('--mode', dest='flowmode', action='store', default='avgflow')
    parser.add_argument('--prev', dest='prev_img', action='store', default='{0}/2011_09_26-1/data/0000000026.png'.format(KITTI_PATH))
    parser.add_argument('--cur', dest='cur_img', action='store', default='{0}/2011_09_26-1/data/0000000027.png'.format(KITTI_PATH))
    ...
    parser.add_argument('--flowname', dest='fn', nargs='?', default='flow',
            help='file name of optical flow')
    (opts, args) = parser.parse_known_args()

    prev = cv2.imread(opts.prev_img, cv2.IMREAD_COLOR)
    cur = cv2.imread(opts.cur_img, cv2.IMREAD_COLOR)
    frame = cur.copy()
    frame = detflow(frame, prev, cur, **vars(opts))
