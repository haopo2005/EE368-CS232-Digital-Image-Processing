...
fontcolors = dict(r=(0,0,255), y=(0,255,255), g=(0,255,0))
    ...
    for cnt in contours:
        ...
        if cond:
            if mode=='compare':
                ...
                if iscv2():
                    cv2.drawContours(img, [cvxhull], contourIdx=-1, color=(0,255,0),
                        thickness=1)
                elif iscv3():
                    img = cv2.drawContours(img, [cvxhull], contourIdx=-1, color=(0,255,0),
                        thickness=1)
            coord = (int(maxX + 4), int((minY + maxY)/2))
