...
def main():
    ...
    # corners of court, computed in ballintersectMain.py
    ...
    for f in range(1, numFrames):
        ...
        if posterPhotos and f == numFrames - 1:
            ...
            # Megan's camera
            fig1.show()
            imFilename = '../jst_output/plot1.png'
            fig1.savefig(imFilename, dpi=200)

            # Kyle's camera
            fig2.show()
            imFilename = '../jst_output/plot2.png'
            fig2.savefig(imFilename, dpi=200)

            # court graphic
            fig3.show()
            imFilename = '../jst_output/plot3.png'
            fig3.savefig(imFilename, dpi=200)

            # 3D scatterplot
            fig4.show()
            imFilename = '../jst_output/plot4.png'
            fig4.savefig(imFilename, dpi=200)

        # update plots
        fig.show()
        imFilename = '../jst_output/image' + str(f) + '.png'
        fig.set_size_inches(12, 8)
