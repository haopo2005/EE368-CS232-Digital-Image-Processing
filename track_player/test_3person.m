clear, close all
vidobj = VideoReader('test_3person.mp4');
...
centroids_p3 = zeros(size(centroids_p1));
for i = 1:numFrames
    %locate players using locate_players function
    frame_col = read(vidobj,i);
    player_locations(:,:,i) = locate_players(frame_col,r_lim,g_lim,b_lim);
...
imwrite(final_im,'3person_morphological_testpass.jpg');

%%
vidwriter = VideoWriter('3person_pathtestmorp.avi');
...
vidwriter_player = VideoWriter('3person_player_tracking.avi');
...
vidwriter_boxes = VideoWriter('3person_player_boxes.mp4','MPEG-4');
...
for i = 1:numFrames
    ...
    pos_p3 = [centroids_p3(i,1)-75 centroids_p3(i,2)-100 150 300];
    pos = vertcat(pos_p1,pos_p2,pos_p3);
...
fig2 = figure,plot(centroids_p1(:,1)',centroids_p1(:,2)','b*',...
    centroids_p2(:,1)',centroids_p2(:,2)','r*',...
    centroids_p3(:,1)',centroids_p3(:,2)','g*','MarkerSize',5)
set(gca, 'YDir','reverse')
xlim([2 1280])
ylim([2 720])
...
%%
[num_passes,player_regions] = count_passes_3person(centroids_disc,centroids_p1,centroids_p2,centroids_p3,...
    numFrames,150,300);
vidwriter_player_reg = VideoWriter('3person_player_regions.mp4','MPEG-4');
